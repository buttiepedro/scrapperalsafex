...
class KnowledgePayloadTest(unittest.TestCase):
    def test_build_payload_splits_documents_and_accessories(self):
        ...
        self.assertNotIn("categoria_slug", payload["documentos"][0])
        self.assertEqual(payload["accesorios"][0]["categoria_slug"], "accesorios")
        self.assertEqual(payload["documentos"][0]["Link"], docs[0].url)
        self.assertEqual(payload["accesorios"][0]["Link"], docs[1].url)
        self.assertNotIn("url", payload["documentos"][0])
        self.assertNotIn("url", payload["accesorios"][0])
        self.assertNotIn("fecha_publicacion", payload["documentos"][0])
        self.assertNotIn("fecha_publicacion", payload["accesorios"][0])
